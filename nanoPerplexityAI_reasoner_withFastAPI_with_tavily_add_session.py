...
class SessionData:
    def __init__(self):
        ...
        self.history = []
        self.file_path = f"sessions/{self.session_id}.md"  # 每个会话独立文件
...
async def read_index():
    return FileResponse('static/index_session.html')
...
@app.post("/ask")
async def ask(
    query: str = Body(..., embed=True),      # 从请求体获取
    session_id: str = Body(..., embed=True)  # 从请求体获取
):
    # 验证会话
    ...
    # 关键修改点：获取AI响应内容
    search_dic = llm_check_search(query, session.file_path, session.history)
    session.history = llm_answer(query, session.file_path, session.history, search_dic)
    ...
    return {
        ...
        "sources": list(search_dic.keys()) if search_dic else []  # 可选：返回参考链接
    }
